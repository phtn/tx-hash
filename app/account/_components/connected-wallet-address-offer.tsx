...
export function ConnectedWalletAddressOffer({
  ...
  if (variant === 'topbar') {
    return (
      <button
        ...
        className={cn(
          'flex h-10 shrink-0 items-center gap-2 rounded-xl border border-accent/35 bg-accent/12 px-3 font-mono text-[10px] uppercase tracking-[0.18em] text-[#18120f] outline-accent transition-colors hover:bg-accent/18 disabled:cursor-default disabled:opacity-70 dark:border-accent/40 dark:bg-accent/18 dark:text-white dark:hover:bg-accent/24',
  ...
  return (
    <section
      className={cn('border-b border-black/8 bg-white/75 p-4 dark:border-background dark:bg-white/4', className)}>
        ...
        <div className='min-w-0'>
          <div className='flex items-center gap-2'>
            <span
              ...
              <Icon name={'eth'} className='size-6' />
            </span>
            <p className='mt-0 font-mono text-[10px] uppercase tracking-[0.22em] text-[#7f7368] dark:text-white/38'>
          ...
          <div className='flex items-center space-x-4'>
            {connectedWallet ? (
              <p className='mt-0 font-mono text-lg text-[#18120f] dark:text-white tracking-widest'>
                {connectedWalletLabel}
              </p>
            ) : (
              <p className='mt-0 text-sm leading-6 text-[#675d53] dark:text-white/58'>
                Connect a wallet and it will appear here as a one-click save option.
              </p>
            )}
            {error ? <p className='mt-0 text-xs leading-5 text-red-700 dark:text-red-300'>{error}</p> : null}
            {connectedWallet ? (
              <button
                type='button'
                onClick={() => void saveConnectedWallet()}
                disabled={isSaved || isSaving || isChecking}
                className={cn(
                  'flex items-center space-x-3 h-6 shrink-0 border border-foreground/5 px-2 font-mono text-[8px] uppercase tracking-[0.22em] text-[#18120f] outline-accent transition-colors hover:bg-accent/18 disabled:cursor-default disabled:opacity-65 dark:text-white dark:hover:bg-accent/24',
                  isSaved && 'space-x-0 border-emerald-500/25 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300'
                )}>
                {!isSaved && <Icon name={'plus'} className='size-4' />}
                <span>{actionLabel}</span>
              </button>
            ) : null}
          </div>
